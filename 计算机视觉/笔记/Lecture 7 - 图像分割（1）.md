...
> [!info] 课程概要
> 本讲从 [[Lecture 3 - 图像处理]] 的灰度变换、滤波与距离变换出发，转向“如何把图像划分为有意义区域”的核心任务。主线可以概括为：**阈值分割 → 分水岭 → 聚类分割 → 活动轮廓**。其中边缘、梯度与轮廓思想又与 [[Lecture 4 - 特征提取]] 直接相连，并为 [[Lecture 8 -.md|Lecture 8 - 图像分割（2）]] 的图模型与后处理打基础。
...
Chan-Vese 可以看作从 Lecture 7 过渡到 [[Lecture 8 -.md|Lecture 8 - 图像分割（2）]] 的关键桥梁：
...
> - 后续 [[Lecture 8 -.md|Lecture 8 - 图像分割（2）]] 会继续进入图模型与后处理
...
*本笔记由 Claudian 整理 | [[Lecture 3 - 图像处理]] → [[Lecture 4 - 特征提取]] → [[Lecture 7 - 图像分割（1）|Lecture 7 - 图像分割（1）]] → [[Lecture 8 -.md|Lecture 8 - 图像分割（2）]]*